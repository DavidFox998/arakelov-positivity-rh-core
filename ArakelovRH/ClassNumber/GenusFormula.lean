/-
  ...
  Formal proof: genus(X_0(143)) = 13.

  Mathematical source: DavidFox998/ClassNumber-143 (read-only reference).
  ...
  nu2 = 0     chi_{-4}(11) = -1, product collapses
  nu3 = 0     chi_{-3}(11) = -1, product collapses
  nu8 = 4     sum over divisors d|143 of phi(gcd(d, 143/d))
  g   = 1 + 14 - 0 - 0 - 2 = 13

  All steps: decide / norm_num.
...
/-! ## Index and cusp-count arithmetic -/

/-- mu = [SL_2(Z) : Gamma_0(143)] = 143 * (12/11) * (14/13) = 168.
    Integer arithmetic: 143*12 = 1716; 1716/11 = 156; 156*14 = 2184; 2184/13 = 168. -/
theorem mu_143 : 143 * 12 / 11 * 14 / 13 = 168 := by decide

/-- nu_inf = sum_{d | 143} phi(gcd(d, 143/d)) for d in {1, 11, 13, 143}.
    = phi(gcd(1,143)) + phi(gcd(11,13)) + phi(gcd(13,11)) + phi(gcd(143,1))
    = phi(1) + phi(1) + phi(1) + phi(1) = 4. -/
theorem nu_inf_143 :
    Nat.totient (Nat.gcd 1 143) + Nat.totient (Nat.gcd 11 13) +
    Nat.totient (Nat.gcd 13 11) + Nat.totient (Nat.gcd 143 1) = 4 := by decide

/-- nu2 = product over p|143 of (1 + chi_{-4}(p)) = 0.
    chi_{-4}(11) = -1 makes (1 + chi_{-4}(11)) = 0 mod 11. -/
theorem nu2_vanishes : (1 : ZMod 11) + (-4 : ZMod 11) ^ ((11-1)/2) = 0 := by decide

/-- nu3 = product over p|143 of (1 + chi_{-3}(p)) = 0.
    chi_{-3}(11) = -1 makes (1 + chi_{-3}(11)) = 0 mod 11. -/
theorem nu3_vanishes : (1 : ZMod 11) + (-3 : ZMod 11) ^ ((11-1)/2) = 0 := by decide

/-! ## Diamond-Shurman genus formula -/
...
/-- **genus(X_0(143)) = 13** (Diamond-Shurman Theorem 3.1.1, N = 143).
    ...
    nu2 = 0: chi_{-4}(11) = -1, product collapses (proved above).
    nu3 = 0: chi_{-3}(11) = -1, product collapses (proved above).
    mu/12 = 168/12 = 14; nu_inf/2 = 4/2 = 2.
...
/-- Agrees with ArakelovRH.C01_Arakelov: X_0(143) has genus 13.
    This is the Diamond-Shurman certification of the C01 datum. -/
